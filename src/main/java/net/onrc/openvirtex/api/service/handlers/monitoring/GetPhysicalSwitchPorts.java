...
public class GetPhysicalSwitchPorts extends ApiHandler<Map<String, Object>> {
    ...
    public JSONRPC2Response process(final Map<String, Object> params) {
        try {
            ...
            LinkedList<PhysicalPort> ports = new LinkedList<PhysicalPort>();

            if (dpid.longValue() == -1) {
                HashMap<String, List<Object>> res = new HashMap<String, List<Object>>();
                for (PhysicalSwitch sw : PhysicalNetwork.getInstance()
                        .getSwitches()) {
                	ports = aggregatePortsBySwitch(sw.getSwitchId(), map);
                    res.put(sw.getSwitchName(), portsToList(ports));
                }
                this.resp = new JSONRPC2Response(res, 0);
            }
//            } else {
//            	ports = aggregatePortsBySwitch(dpid.longValue(), map);
//            	
...
//            }
    ...
    private List<Object> portsToList(LinkedList<PhysicalPort> ports) {
        final List<Object> res = new LinkedList<Object>();
        for (PhysicalPort p : ports) {
            res.add(p.convertToPhysical());
        }
        return res;
    }

    private LinkedList<PhysicalPort> aggregatePortsBySwitch(
            long dpid, Mappable map) {
        LinkedList<PhysicalPort> ports = new LinkedList<PhysicalPort>();
        final PhysicalSwitch sw = PhysicalNetwork.getInstance().getSwitch(dpid);
        ports.addAll(sw.getPorts().values());
